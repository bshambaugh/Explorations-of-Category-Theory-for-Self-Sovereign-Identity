...
# Look at Groups Applied to Cryptography:
  - See Elliptic Curve Cryptography in Practical Cryptography for Developers:
     ...
     Elliptic Curves are defined over finite fields. 

     The ECSA Key-Pair consists of a private Key (privKey) that is an integer in the range [0..n-1], and a public key (pubKey) that is an EC-Point equal to the private key multiplied by a generator point (G). pubKey = privKey * G.
     n is the order of the subgroup generated by G. The public point and the generator point are both on a curve. For the case of (mention other ECC curves also] P256 the Weierstrauss form is followed.
